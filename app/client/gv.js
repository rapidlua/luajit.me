...
export function jsonGetExtents(json) {
  const [llx, lly, urx, ury] = json.bb.split(',');
  return [/*width*/ urx - llx, /*height*/ lly - ury];
}

export function jsonGetSVGAttrs(json, options) {
...
export function jsonCreateSVGRenderer(target) {
  const context = {};
